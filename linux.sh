...
Resolve-DnsName -Name "172.x.x.x" -Type PTR | Select-Object -ExpandProperty NameHost
...
each mail tool has its way of checking if it exists using "which <key>": mailutils, postfix, ssmtp
	to list all mail tools:
		dpkg -l | grep mail
